...
L.Layer.MyTracksVector = L.VectorGrid.Protobuf.extend({
    ...
    _makeTrackInfo: function(properties) {
        ...
        if ('ascent' in properties && 'descent' in properties) {
            info += `&nbsp;(&#8645;${properties.ascent}/${properties.descent}&nbsp;m)`;
        }
        if ('moving_time' in properties && 'stopped_time' in properties) {
            const moving_time_h = Math.floor(properties.moving_time / 3600);
            const moving_time_min = Math.floor((properties.moving_time % 3600) / 60);
            const stopped_time_h = Math.floor(properties.stopped_time / 3600);
            const stopped_time_min = Math.floor((properties.stopped_time % 3600) / 60);
            info += `<br/>Moving time: ${moving_time_h}h ${moving_time_min}min`;
            info += `<br/>Stopped time: ${stopped_time_h}h ${stopped_time_min}min`;
        }
